#### This script generates 500 diversity samples of 120 languages from the languages in the GBI logical dataset.
...
expectations <- read.csv("scripts/GBI/decisions-log.csv") %>% filter(modification.type == "statistical")

# load logical grambank data, glottolog taxonomy and glottolog macroarea data
logical_data <- read.csv("curated_data/GBI/logicalGBI/logicalGBI.csv") %>% select(-X)
taxonomy <- read.csv("curated_data/GBI/logicalGBI/cldf/languages.csv")
...
# load data for stats
recoded_data <- read.csv("curated_data/GBI/statisticalGBI/data_for_stats.csv") %>% select(-X)
...
write.csv(lg_samples_500,"curated_data/GBI/500_diversity_samples.csv")
