#### This script generates 1000 diversity samples of 120 languages from the languages in the TLI logical dataset.
...
expectations <- read.csv("scripts/TLI/decisions-log.csv") %>% filter(modification.type == "statistical")

# load logical grambank data, glottolog taxonomy and glottolog macroarea data
logical_data <- read.csv("curated_data/TLI/logicalTLI/full/logicalTLI_full.csv") %>% select(-X)
taxonomy <- read.csv("curated_data/TLI/logicalTLI/cldf/languages.csv")
...
# load data for stats
recoded_data <- read.csv("curated_data/TLI/statisticalTLI/data_for_stats.csv") %>% select(-X)
...
write.csv(lg_samples_1000,"curated_data/TLI/1000_diversity_samples.csv")
...
# run all tests
results <- evaluate_OR_AND_THEN(recoded_data = recoded_data, 
