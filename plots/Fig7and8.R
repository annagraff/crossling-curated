...
sum(!is.na(phonology_logical_pruned))/(nrow(phonology_logical_pruned)*ncol(phonology_logical_pruned)) # coding density: 61% --> ok!

phonology_statistical_pruned <- read.csv("../data/TypLinkInd/output/statisticalTLI/phonology/statisticalTLI_phonology_pruned.csv", row.names = "X")
phonology_statistical_pruned <- na_convert(phonology_statistical_pruned)
phonology_statistical_pruned <- factorise(phonology_statistical_pruned)
sum(!is.na(phonology_statistical_pruned))/(nrow(phonology_statistical_pruned)*ncol(phonology_statistical_pruned)) # coding density: 60% --> ok!
...
sum(!is.na(lexicon_logical_pruned))/(nrow(lexicon_logical_pruned)*ncol(lexicon_logical_pruned)) # coding density: 53% --> ok!

lexicon_statistical_pruned <- read.csv("../data/TypLinkInd/output/statisticalTLI/lexicon/statisticalTLI_lexicon_pruned.csv",row.names = "X")
...
sum(!is.na(lexicon_statistical_pruned))/(nrow(lexicon_statistical_pruned)*ncol(lexicon_statistical_pruned)) # coding density: 53% --> ok!
...
phonology_logical_taxonomy <- taxonomy_matrix_for_plot %>% filter(id %in% rownames(phonology_logical_pruned)) %>% filter(!is.na(lon))
phonology_statistical_taxonomy <- taxonomy_matrix_for_plot %>% filter(id %in% rownames(phonology_statistical_pruned)) %>% filter(!is.na(lon))
...
phonology_logical_with_lg_coords <- phonology_logical_pruned[which(rownames(phonology_logical_pruned)%in%phonology_logical_taxonomy$id),]
phonology_statistical_with_lg_coords <- phonology_statistical_pruned[which(rownames(phonology_statistical_pruned)%in%phonology_statistical_taxonomy$id),]
...
gbi_comparison_plot <- grid_point_comparison_horizontal(baseline_gp_frame = gb_original_gps[[1]], 
                                                       ...
                                                       world_map = world_map_initial)
# ggsave(file="Fig7.png",gbi_comparison_plot[[1]])
...
ggsave(file="Fig8.png",entropies)
