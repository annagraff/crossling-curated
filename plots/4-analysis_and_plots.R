...
#### Gower distances and neighbour-nets
# compute gower distances, using daisy()
full_statistical_small_gower_test <- daisy(full_statistical_small_with_lg_coords, metric = "gower") %>% as.matrix()
expect_true(any(full_statistical_small_gower_test==0))

# there are some zero distances between lanuages due to remaining patchiness. we identify them and remove the languages necessary to then recreate the distance matrix
diag(full_statistical_small_gower_test) <- NA
full_statistical_small_gower_test[upper.tri(full_statistical_small_gower_test, diag = TRUE)] <- NA

# Convert cell indices to row and column indices
duplicates <- data.frame(lg1 = rownames(full_statistical_small_gower)[(which(full_statistical_small_gower_test == 0) - 1) %/% ncol(full_statistical_small_gower) + 1], 
               lg2 = colnames(full_statistical_small_gower)[(which(full_statistical_small_gower_test == 0) - 1) %% ncol(full_statistical_small_gower) + 1])

duplicates

# here, removing bard1255, dong1288 and sout2991 removes all 0 distances!
data_for_distance <- full_statistical_small_with_lg_coords[-(which(rownames(full_statistical_small_with_lg_coords)%in%c("bard1255","dong1288","sout2991"))),]
gower <- daisy(data_for_distance, metric = "gower") %>% as.matrix()

# there should be no zeros left, and there aren't
diag(gower) <- NA
expect_false(any(gower[which(gower == 0)]))

# save gower to create neighbor-joining tree in SplitsTree4
write.nexus.dist(gower, "output/statisticalMM/full/dist_for_splitstree4.nex")

a<-read.nexus.networx("output/statisticalMM/full/initialmadness.nex")
b<-read.nexus.networx("output/statisticalMM/full/madness.txt")

repayment <- function(debt, interest=1.075, monthly = 500){
  i <- debt
  year <- 0
  while(i>0){
    print(c(year,i))
    i <- i*interest-12*monthly
    year <- year+1 
  }
  return(year)
}
...
library(tanggle)
library(phangorn)
full_statistical_small_neighbournet <- splitstree(full_statistical_small_gower) 
