...
rgb_mapping_gbi_logical_densified <- RGB_mapping(per_family_props_PCA_gbi_logical_densified,taxonomy_matrix_for_plot, pc1flip = T, pc2flip = F, pc3flip = F) %>% filter(id%in%rownames(gbi_logical_densified)) %>% na.omit()
rgb_mapping_tli_logical_large <- RGB_mapping(per_family_props_PCA_tli_logical_large,taxonomy_matrix_for_plot, pc1flip = F, pc2flip = T, pc3flip = F) %>% filter(id%in%rownames(tli_logical_large)) %>% na.omit()
...
rgb_mapping_phonology_logical_pruned <- RGB_mapping(per_family_props_PCA_phonology_logical_pruned,taxonomy_matrix_for_plot, pc1flip = T, pc2flip = F, pc3flip = F) %>% filter(id%in%rownames(phonology_logical_pruned)) %>% na.omit()
rgb_mapping_lexicon_logical_pruned <- RGB_mapping(per_family_props_PCA_lexicon_logical_pruned,taxonomy_matrix_for_plot, pc1flip = T, pc2flip = T, pc3flip = F) %>% filter(id%in%rownames(lexicon_logical_pruned)) %>% na.omit()
