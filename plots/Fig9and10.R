...
gbi_logical_densified <- read.csv("../data/GBInd/output/logicalGBI/logicalGBI_pruned.csv", row.names = "X")
...
tli_logical_large <- read.csv("../data/TypLinkInd/output/logicalTLI/full/logicalTLI_full_pruned_large.csv", row.names = "X")
tli_logical_large <- na_convert(tli_logical_large)
tli_logical_large <- factorise(tli_logical_large)

tli_logical_small <- read.csv("../data/TypLinkInd/output/logicalTLI/full/logicalTLI_full_pruned_small.csv", row.names = "X")
tli_logical_small <- na_convert(tli_logical_small)
tli_logical_small <- factorise(tli_logical_small)

# morphosyntax
morphosyntax_logical_large <- read.csv("../data/TypLinkInd/output/logicalTLI/morphosyntax/logicalTLI_morphosyntax_pruned_large.csv", row.names = "X")
...
morphosyntax_logical_small <- read.csv("../data/TypLinkInd/output/logicalTLI/morphosyntax/logicalTLI_morphosyntax_pruned_small.csv", row.names = "X")
...
# phonology
phonology_logical_pruned <- read.csv("../data/TypLinkInd/output/logicalTLI/phonology/logicalTLI_phonology_pruned.csv", row.names = "X")
phonology_logical_pruned <- na_convert(phonology_logical_pruned)
phonology_logical_pruned <- factorise(phonology_logical_pruned)

# lexicon
lexicon_logical_pruned <- read.csv("../data/TypLinkInd/output/logicalTLI/lexicon/logicalTLI_lexicon_pruned.csv", row.names = "X")
...
rgb_mapping_gbi_logical_full <- RGB_mapping(per_family_props_PCA_gbi_logical_full,taxonomy_matrix_for_plot, pc1flip = T) %>% filter(id%in%rownames(gbi_logical_full)) %>% na.omit()
rgb_mapping_gbi_logical_densified <- RGB_mapping(per_family_props_PCA_gbi_logical_densified,taxonomy_matrix_for_plot) %>% filter(id%in%rownames(gbi_logical_densified)) %>% na.omit()
rgb_mapping_tli_logical_large <- RGB_mapping(per_family_props_PCA_tli_logical_large,taxonomy_matrix_for_plot, pc2flip = T, pc3flip = T) %>% filter(id%in%rownames(tli_logical_large)) %>% na.omit()
rgb_mapping_tli_logical_small <- RGB_mapping(per_family_props_PCA_tli_logical_small,taxonomy_matrix_for_plot, pc2flip = T, pc3flip = T) %>% filter(id%in%rownames(tli_logical_small)) %>% na.omit()
rgb_mapping_morphosyntax_logical_large <- RGB_mapping(per_family_props_PCA_morphosyntax_logical_large,taxonomy_matrix_for_plot, pc1flip = T, pc2flip = T, pc3flip = T) %>% filter(id%in%rownames(morphosyntax_logical_large)) %>% na.omit()
rgb_mapping_morphosyntax_logical_small <- RGB_mapping(per_family_props_PCA_morphosyntax_logical_small,taxonomy_matrix_for_plot, pc1flip = T, pc2flip = T, pc3flip = T) %>% filter(id%in%rownames(morphosyntax_logical_small)) %>% na.omit()
rgb_mapping_phonology_logical_pruned <- RGB_mapping(per_family_props_PCA_phonology_logical_pruned,taxonomy_matrix_for_plot, pc1flip = T,) %>% filter(id%in%rownames(phonology_logical_pruned)) %>% na.omit()
rgb_mapping_lexicon_logical_pruned <- RGB_mapping(per_family_props_PCA_lexicon_logical_pruned,taxonomy_matrix_for_plot) %>% filter(id%in%rownames(lexicon_logical_pruned)) %>% na.omit()

# global PC maps for PCs 1-3
rgb_map_gbi_logical_full <- rgb_map(rgb_mapping_gbi_logical_full, world_map_initial, main="GBInd", plot=F, size = 1.2, pca.object = per_family_props_PCA_gbi_logical_full)
rgb_map_gbi_logical_densified <- rgb_map(rgb_mapping_gbi_logical_densified, world_map_initial, main="GBInd (densified)", plot=F, size = 1.2, pca.object = per_family_props_PCA_gbi_logical_densified)
...
rgb_map_morphosyntax_logical_small <- rgb_map(rgb_mapping_morphosyntax_logical_small, world_map_initial, main="TypLinkInd [morphosyntax]", plot=F, size = 1.2, pca.object = per_family_props_PCA_morphosyntax_logical_small)
...
f2d_morphosyntax_logical_small <- dim_mapping(pca.object = per_family_props_PCA_morphosyntax_logical_small, taxonomy_matrix = taxonomy_matrix_for_plot, size = 1.2)
...
logical_plots <- grid.arrange(
  arrangeGrob(rgb_map_gbi_logical_full, f2d_full_gbi_logical_full, rgb_map_morphosyntax_logical_large, f2d_morphosyntax_logical_large, nrow = 2, ncol = 2, widths=c(3,2)),
  arrangeGrob(rgb_map_phonology_logical_pruned, f2d_phonology_logical_pruned, rgb_map_lexicon_logical_pruned, f2d_lexicon_logical_pruned, nrow = 2, ncol = 2, widths=c(3,2)),
  nrow = 2
)
ggsave("Fig9.png", plot = logical_plots, width = 14, height = 8*2, dpi = 300)


logical_plots_densify <- grid.arrange(
  arrangeGrob(rgb_map_gbi_logical_full, f2d_full_gbi_logical_full, rgb_map_gbi_logical_densified, f2d_full_gbi_logical_densified, nrow = 2, ncol = 2, widths=c(3,2)),
...
ggsave("Fig10.png", plot = logical_plots_densify, width = 14, height = 8*2, dpi = 300)
