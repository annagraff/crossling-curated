...
# read in logical GBM data
logical <- read.csv("output/logicalGBM/logicalGBM.csv") %>% select(-X)
lgs_lo <- logical$glottocode
logical <- logical %>% select(-glottocode)
rownames(logical) <- lgs_lo

# read in statistical GBM data
statistical <- read.csv("output/statisticalGBM/statisticalGBM.csv") %>% select(-X)
...
# subset logical and statistical GBM data to those languages with language coordinates (for plotting later)
...
write.csv(pruned_logical,"output/logicalGBM/logicalGBM_pruned.csv")
write.csv(pruned_statistical,"output/statisticalGBM/statisticalGBM_pruned.csv")
