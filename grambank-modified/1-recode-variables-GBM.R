...
original_variable_matrix <- as.data.frame(read_csv("../input/grambank_v.1.0.3.csv", 
                                                   trim_ws = FALSE, col_types = "f"))

names(original_variable_matrix)[1]<-"glottocode"
...
########## recode group 2 (merge variables - recode via logical arguments) ########## 
# subset to features that have to be merged via logical arguments
second_set <- filter(recode_patterns, recode.operation.type=="recode group 2 (merge variables - recode via logical arguments)")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 2 (merge variables - recode via logical arguments)")
...
########## recode group 3 (merge variables - recode via logical arguments - simple conditioning) ########## 
# subset to features that have to be recoded via logical arguments if a condition applies
third_set <- filter(recode_patterns, recode.operation.type=="recode group 3 (merge variables - recode via logical arguments - simple conditioning)")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 3 (merge variables - recode via logical arguments - simple conditioning)")
...
########## recode group 4 (merge variables - recode via logical arguments - multiple conditioning) ########## 
# subset to features that have to be recoded via logical arguments if several conditions apply
fourth_set <- filter(recode_patterns, recode.operation.type=="recode group 4 (merge variables - recode via logical arguments - multiple conditioning)")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 4 (merge variables - recode via logical arguments - multiple conditioning)")
...
########## recode group 7 (simple conditioning [conditioned variable]) ########## 
# subset to features that have to be conditioned on another conditioned feature
seventh_set <- filter(recode_patterns, recode.operation.type=="recode group 7 (simple conditioning [conditioned variable])")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 7 (simple conditioning [conditioned variable])")
...
########## recode group 8 (multiple conditioning [conditioned variable]) ########## 
# subset to features that have to be conditioned on several features
eighth_set <- filter(recode_patterns, recode.operation.type=="recode group 8 (multiple conditioning [conditioned variable])")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 8 (multiple conditioning [conditioned variable])")
...
########## recode group 9 (merge variables - recode via logical arguments - simple conditioning [conditioned variable]) ########## 
# subset to features that have to be recoded via logical arguments if a condition applies (conditioned variable)
ninth_set <- filter(recode_patterns, recode.operation.type=="recode group 9 (merge variables - recode via logical arguments - simple conditioning [conditioned variable])")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 9 (merge variables - recode via logical arguments - simple conditioning [conditioned variable])")
...
########## recode group 10 (merge variables - recode via logical arguments - multiple conditioning [conditioned variable]) ########## 
# subset to features that have to be recoded via logical arguments if a condition applies (conditioned variable)
tenth_set <- filter(recode_patterns, recode.operation.type=="recode group 10 (merge variables - recode via logical arguments - multiple conditioning [conditioned variable])")
recode_patterns <- filter(recode_patterns, recode.operation.type!="recode group 10 (merge variables - recode via logical arguments - multiple conditioning [conditioned variable])")
...
write.csv(logical_data,"output/logicalGBM/logicalGBM.csv")

statistical_data <- recoded_data %>% select(c("glottocode",statistical_layer$new.name))
write.csv(statistical_data,"output/statisticalGBM/statisticalGBM.csv")
...
recoded_lgs <- logical_data$glottocode
...
write.csv(lg_samples_500,"output/500_diversity_samples_seed_2023.csv")
...
write.csv(taxonomy_logical,"output/logicalGBM/cldf/languages.csv",fileEncoding="UTF-8",row.names = F)
...
write.csv(logical_long,"output/logicalGBM/cldf/values.csv",fileEncoding="UTF-8",row.names = F)
...
write.csv(statistical_long,"output/statisticalGBM/cldf/values.csv",fileEncoding="UTF-8",row.names = F)

# codes.csv
logical_codes <- logical_long %>% select(c("code_ID","new.name","value")) %>% unique()
write.csv(logical_codes,"output/logicalGBM/cldf/codes.csv",fileEncoding="UTF-8",row.names = F)
